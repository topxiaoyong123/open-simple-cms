 ...
 */
public class ContentPublishingListPanel extends com.extjs.gxt.ui.client.widget.ContentPanel {
    ...
    public ContentPublishingListPanel(final Content content) {
        if (service == null) {
        ...
        publish.addSelectionListener(new SelectionListener() {
            public void componentSelected(ComponentEvent ce) {
                if(smm.getSelectedItems().size() <= 0){
                    ...
                    Dispatcher.forwardEvent(AppEvents.CONTENT_PUBLISHING_MANAGER_DO, contents);
                }
            }
        });
        ...
        MenuItem publishingMenu = new MenuItem();
        publishingMenu.setText(msgs.content_publishing_do());
        publishingMenu.setIconStyle("article-wenzhangfubu");
        publishingMenu.addSelectionListener(new SelectionListener() {
            public void componentSelected(ComponentEvent ce) {
                if (smm.getSelectedItems().size() <= 0) {
                    MessageBox.alert(msgs.warn(), msgs.choose(), null);
                } else {
        ...
        contextMenu.add(publishingMenu);
